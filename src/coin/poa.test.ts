...
import { decodePoaAddress, encodePoaAddress } from "./poa.js";
...
])("poa address", ({ text, hex }) => {
  test(`encode: ${text}`, () => {
    expect(encodePoaAddress(hexToBytes(hex))).toEqual(text);
  });
  test(`decode: ${text}`, () => {
    expect(decodePoaAddress(text)).toEqual(hexToBytes(hex));
  });
});
